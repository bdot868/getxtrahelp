...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest') {
        ...
        error_log("\n\n -------------------------------------" . date('c'). " \n" .$pageURL. " \n" . file_get_contents('php://input'), 3, FCPATH.'worker/api_log-'.date('d-m-Y').'.log');
    ...
    public function response($data = NULL, $http_code = NULL, $continue = FALSE) {
        ...
        $data = $this->cleanArrayOutPut($data);
    ...
    public function checkUserRequest () {
        $requestHeader = $this->input->request_headers();
        /*
        if (!isset($requestHeader['pnp_access_key']) || $this->config->item('rest_api_auth_key') != $requestHeader['pnp_access_key']) {
            $response['status'] = "false";
            $response['error'] = 'Invalid API key';
            return $this->response($response, REST_Controller::HTTP_OK);
        }*/
        $apiData = json_decode(file_get_contents('php://input'), TRUE);
        if($this->input->post('langType') != ""){
            $apiData['data']['langType'] = $this->input->post('langType');
        }
        
        if($this->input->post('token') != ""){
            $apiData['data']['token'] = $this->input->post('token');
        }
        
        if (!isset($apiData['data']['langType']) || empty($apiData['data']['langType'])) {
            $response['message'] = $this->Common_Model->GetNotification("passlang", 1);
            return $this->response($response, REST_Controller::HTTP_OK);
        }

        if (!isset($apiData['data']['token']) || empty($apiData['data']['token'])) {
            $response['status'] = "2";
            $response['message'] = $this->Common_Model->GetNotification("tokenRequired", $apiData['data']['langType']);
            return $this->response($response, REST_Controller::HTTP_OK);
        }

        $auth = $this->db->get_where('tbl_auth', array('token' => $apiData['data']['token']))->row();
        if (empty($auth)) {
            $response['status'] = "2";
            $response['message'] = $this->Common_Model->GetNotification("authFailRequired", $apiData['data']['langType']);
            return $this->response($response, REST_Controller::HTTP_OK);
        }
        $user = $this->db->get_where('tbl_users', array('id' => $auth->userId))->row();
        if (empty($user)) {
            $response['status'] = "2";
            $response['message'] = $this->Common_Model->GetNotification("authFailRequired", $apiData['data']['langType']);
            return $this->response($response, REST_Controller::HTTP_OK);
        ...
        if ($user->status == 2) {
            $response['status'] = "5";
            $response['message'] = $this->Common_Model->GetNotification("accountBlockedByAdmin", $apiData['data']['langType']);
            return $this->response($response, REST_Controller::HTTP_OK);
        }
        if ($user->status == 3) {
            $response['status'] = "2";
            $response['message'] = $this->Common_Model->GetNotification("userDeletedByAdmin", $apiData['data']['langType']);
            return $this->response($response, REST_Controller::HTTP_OK);
        }
        if(!isset($user->timezone) || empty($user->timezone)){
            $user->timezone = $this->config->item('time_reference');
        }
    ...
    public function checkGuestUserRequest () {
    ...
    public function cleanArrayOutPut($data) {
        //convert to string if data is not array & object and retuen data
        if (!is_array($data) && !is_object($data)) {
            return (string) $data;
        }
        
        foreach ($data as $key => $value) {
            //if value is an stdobject type than conver to array
            if (is_object($value)) {
                $value = (array) $value;
            }
            
            //if value type is array than call cleanArrayOutPut function to clean
            if (is_array($value)) {
