...
class User_Feed_Like_Model extends CI_Model {

    public function __construct() {
        parent::__construct(); 
        $this->table = "tbl_user_feed_like";
    ...
    public function get($data = [], $single = false, $num_rows = false) {
        ...
        if ($num_rows) {
            ...
            if(isset($data['apiResponse'])){
                $this->db->select($this->table . '.id as userFeedLikeId');
        ...
        if(isset($data['feedToId']) && $data['feedToId'] == true){
            $this->db->select($this->table . '.userFeedId as id');
        }
