...
class Job_Sub_Category_Model extends CI_Model {
    ...
    public function __construct() {
        parent::__construct(); 
        $this->table = "tbl_job_sub_category";
    ...
    public function get($data = [], $single = false, $num_rows = false) {
        ...
        if ($num_rows) {
            ...
            if(isset($data['apiResponse'])){
                $this->db->select($this->table . '.id as jobSubCategoryId');
        ...
        if (isset($data['getParentCatName']) && $data['getParentCatName'] == true){
            $this->db->select($this->tbl_job_category . '.name as parentCatName');
            $this->db->join($this->tbl_job_category, $this->table.'.jobCategoryId = '.$this->tbl_job_category.'.id AND '.$this->tbl_job_category.'.status = 1','left');
        }
