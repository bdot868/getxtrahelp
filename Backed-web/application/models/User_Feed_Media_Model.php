...
class User_Feed_Media_Model extends CI_Model {

    public function __construct() {
        parent::__construct(); 
        $this->table = "tbl_user_feed_media";
    }
    ...
    public function get($data = [], $single = false, $num_rows = false) {
        ...
        if ($num_rows) {
            ...
            if(isset($data['apiResponse'])){
                $this->db->select($this->table . '.id as userFeedMediaId');
    ...
    public function setData($data, $id = 0) {
        ...
        if (empty($id) && (!isset($data['userFeedIds']) && empty($data['userFeedIds']))) {
        ...
        if (!empty($id)) {
            ...
            if (isset($data['userFeedIds']) && !empty($data['userFeedIds'])) {
                $this->db->where('userFeedId', $data['userFeedIds']);
                $this->db->update($this->table, $modelData);
            } else {
