...
class User_Work_Experience_Model extends CI_Model {

    public function __construct() {
        parent::__construct(); 
        $this->table = "tbl_user_work_experience";
    }
    ...
    public function get($data = [], $single = false, $num_rows = false) {
        ...
        if ($num_rows) {
            ...
            if(isset($data['apiResponse'])){
                $this->db->select($this->table . '.id as userWorkExperienceId');
                $this->db->select($this->table . '.userId');
                $this->db->select($this->table . '.workPlace');
                $this->db->select($this->table . '.startDate');
                $this->db->select($this->table . '.endDate');
                $this->db->select($this->table . '.leavingReason');
        ...
        if (isset($data['search']) && !empty($data['search'])) {
            ...
            $this->db->group_start();
            $this->db->like($this->table . '.workPlace', $search);
            $this->db->like($this->table . '.leavingReason', $search);
        ...
        if (isset($data['workPlace'])) {
            $this->db->where($this->table . '.workPlace', $data['workPlace']);
        }

        if (isset($data['startDate'])) {
            $this->db->where($this->table . '.startDate', $data['startDate']);
        }

        if (isset($data['endDate'])) {
            $this->db->where($this->table . '.endDate', $data['endDate']);
        }

        if (isset($data['leavingReason'])) {
            $this->db->where($this->table . '.leavingReason', $data['leavingReason']);
        }
    ...
    public function setData($data, $id = 0) {
        ...
        if (isset($data['workPlace'])) {
            $modelData['workPlace'] = $data['workPlace'];
        }

        if (isset($data['startDate'])) {
            $modelData['startDate'] = $data['startDate'];
        }

        if (isset($data['endDate'])) {
            $modelData['endDate'] = $data['endDate'];
        }

        if (isset($data['leavingReason'])) {
            $modelData['leavingReason'] = $data['leavingReason'];
        }
