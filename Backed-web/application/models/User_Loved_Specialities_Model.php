...
class User_Loved_Specialities_Model extends CI_Model {

    public function __construct() {
        parent::__construct(); 
        $this->table = "tbl_user_loved_specialities";
        $this->tbl_loved_specialities = "tbl_loved_specialities";
    }
    ...
    public function get($data = [], $single = false, $num_rows = false) {
        ...
        if ($num_rows) {
            ...
            if(isset($data['apiResponse'])){
                $this->db->select($this->table . '.id as userLovedSpecialitiesId');
                $this->db->select($this->table . '.userId');
                $this->db->select($this->table . '.lovedSpecialitiesId');
            }else{
        ...
        if (isset($data['getLovedSpecialitiesData']) && $data['getLovedSpecialitiesData'] == true) {
            $this->db->select($this->tbl_loved_specialities . '.name as lovedSpecialitiesName');
            $this->db->join($this->tbl_loved_specialities, $this->tbl_loved_specialities.'.id = '.$this->table.'.lovedSpecialitiesId AND '.$this->tbl_loved_specialities.'.status = 1','left');
        }
        ...
        if (isset($data['lovedSpecialitiesId'])) {
            $this->db->where($this->table . '.lovedSpecialitiesId', $data['lovedSpecialitiesId']);
        }
    ...
    public function setData($data, $id = 0) {
        ...
        if (isset($data['lovedSpecialitiesId'])) {
            $modelData['lovedSpecialitiesId'] = $data['lovedSpecialitiesId'];
        }
