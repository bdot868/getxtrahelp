...
class User_Feed_Comment_Model extends CI_Model {

    public function __construct() {
        parent::__construct(); 
        $this->table = "tbl_user_feed_comment";
    ...
    public function get($data = [], $single = false, $num_rows = false) {
        ...
        if ($num_rows) {
            ...
            if(isset($data['apiResponse'])){
                $this->db->select($this->table . '.id as userFeedCommentId');
                $this->db->select($this->table . '.userFeedId');
                $this->db->select($this->table . '.userId');
                $this->db->select($this->table . '.comment');
        ...
        if(isset($data['timeFormate']) && $data['timeFormate'] == true){
            $this->db->select('FROM_UNIXTIME(' . $this->table . '.createdDate, "%d %a %Y") as time_ago');
        }
        ...
        if (isset($data['comment'])) {
            $this->db->where($this->table . '.comment', $data['comment']);
        }
    ...
    public function setData($data, $id = 0) {
        ...
        if (isset($data['comment'])) {
            $modelData['comment'] = $data['comment'];
        }
