...
class User_Job_Substitute_Request_Model extends CI_Model {

    public function __construct() {
        parent::__construct(); 
        $this->table = "tbl_user_job_substitute_request";
    ...
    public function get($data = [], $single = false, $num_rows = false) {
        ...
        if ($num_rows) {
            ...
            if(isset($data['apiResponse'])){
                $this->db->select($this->table . '.id as userJobSubstituteRequestId');
                $this->db->select($this->table . '.send_from');
                $this->db->select($this->table . '.send_to');
                $this->db->select($this->table . '.send_user');
                $this->db->select($this->table . '.caregiverStatus');
                $this->db->select($this->table . '.userStatus');
                $this->db->select($this->table . '.jobId');
                $this->db->select($this->table . '.jobId as userJobId');
        ...
        if (isset($data['send_user'])) {
            $this->db->where($this->table . '.send_user', $data['send_user']);
        }
        ...
        if (isset($data['caregiverStatus'])) {
            $this->db->where($this->table . '.caregiverStatus', $data['caregiverStatus']);
        }
     
        if (isset($data['userStatus'])) {
            $this->db->where($this->table . '.userStatus', $data['userStatus']);
        }
    ...
    public function setData($data, $id = 0) {
        ...
        if (isset($data['send_user'])) {
            $modelData['send_user'] = $data['send_user'];
        }
        ...
        if (isset($data['caregiverStatus'])) {
            $modelData['caregiverStatus'] = $data['caregiverStatus'];
        }

        if (isset($data['userStatus'])) {
            $modelData['userStatus'] = $data['userStatus'];
        }
