...
class User_Availability_Setting_New_Model extends CI_Model {

    public function __construct() {
        parent::__construct();
        $this->table = "tbl_user_availability_setting_new";
    }
    ...
    public function get($data = [], $single = false, $num_rows = false) {
        ...
        if ($num_rows) {
            ...
            if(isset($data['apiResponse'])){
                $this->db->select($this->table . '.id as userAvailabilitySettingNewId');
                $this->db->select($this->table . '.userId');
                $this->db->select($this->table . '.type');
                $this->db->select($this->table . '.repeatType');
                $this->db->select($this->table . '.timing');
                $this->db->select($this->table . '.date');
        ...
        if (isset($data['repeatType'])) {
            $this->db->where($this->table . '.repeatType', $data['repeatType']);
        }
        ...
        if (isset($data['date'])) {
            $this->db->where($this->table . '.date', $data['date']);
        }
    ...
    public function setData($data, $id = 0) {
        ...
        if (isset($data['repeatType'])) {
            $modelData['repeatType'] = $data['repeatType'];
        }
