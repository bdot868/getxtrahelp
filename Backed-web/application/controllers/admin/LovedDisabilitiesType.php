...
class LovedDisabilitiesType extends MY_Controller {
    ...
    public function __construct() {
        ...
        $this->data['page'] = ['name'=>"Loved Disabilities Type",'url'=>base_url('admin/lovedDisabilitiesType'),'menu'=>'lovedDisabilitiesType','submenu'=>''];
        $this->data['pageTitle'] = ['title' => 'Loved Disabilities Type', 'icon' => ''];
    }
    ...
    public function index(){
        ...
        $this->data['pagename'] = 'Loved Disabilities Type';
        $this->data['data'] = []; 
    	$this->template->content->view('admin/lovedDisabilitiesType/manage', $this->data);
    ...
    public function getList()
    {
        ...
        $query['status'] = [0,1];
        $resultData =  $this->Loved_Disabilities_Type_Model->get(array_merge($paggination,$query));
        $totalData =  $this->Loved_Disabilities_Type_Model->get($query, false, true);
        $result['data']=array();
    ...
    public function set($id = null)
    {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if(isset($data['id']) && !empty($data['id'])){
                    $this->Loved_Disabilities_Type_Model->setData($data, $data['id']);
            }else{
                    $this->Loved_Disabilities_Type_Model->setData($data);
            }
            $this->session->set_flashdata('success', 'Data Saved');
            return redirect('admin/lovedDisabilitiesType');
        }

        if (!empty($id) && $id != 0) {
            $this->data['data'] = $this->Loved_Disabilities_Type_Model->get(['id' => $id], true);
        ...
        $this->data['pagename'] = 'Set Loved Disabilities'; 
        $this->template->content->view('admin/lovedDisabilitiesType/form', $this->data);
        $this->template->publish();
    }
    ...
    public function delete($id = 0){
        $data['status'] = "2";
        if (!empty($id) && $id != 0) {
            $response = $this->Loved_Disabilities_Type_Model->setData($data, $id);
        } 
        if ($response != "") {
    ...
    public function checkLovedDisabilitiesType(){
        if($this->input->server('REQUEST_METHOD') == 'POST'){
            $data = $this->input->post();
            $request = ['name' => $data['name'],'status' => [0, 1]];
            
            $lovedDisDb = $this->Loved_Disabilities_Type_Model->get($request, true);

            if(isset($data['lovedDisId']) && !empty($data['lovedDisId'])){
                    
                if(!empty($lovedDisDb) && ($data['lovedDisId'] != $lovedDisDb->id)){
                    echo 'false';
                }else{
                    echo 'true';
                }
            }else{
                    echo (!empty($lovedDisDb))?'false':'true';    
            }
            exit;
        }
    }   
