...
class WorkSpeciality extends MY_Controller {
    ...
    public function __construct() {
        ...
        $this->data['page'] = ['name'=>"Work Speciality",'url'=>base_url('admin/workSpeciality'),'menu'=>'WorkSpeciality','submenu'=>'Insurance'];
        $this->data['pageTitle'] = ['title' => 'Work Speciality', 'icon' => ''];
    }
    ...
    public function index(){
        ...
        $this->data['pagename'] = 'Work Speciality List';
        $this->data['data'] = [];
    	$this->template->content->view('admin/workSpeciality/manage', $this->data);
    ...
    public function getList() {
        ...
        $query['status'] = [0,1];
        $resultData =  $this->Work_Speciality_Model->get(array_merge($paggination,$query));
        $totalData =  $this->Work_Speciality_Model->get($query, false, true);
        $result['data'] = [];
    ...
    public function set($id = null) {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if (isset($data['id']) && !empty($data['id'])) {
                $this->Work_Speciality_Model->setData($data, $data['id']);
            } else {
                $this->Work_Speciality_Model->setData($data);
            }
            $this->session->set_flashdata('success', 'Data Saved');
            return redirect('admin/workSpeciality');
        }

        if (!empty($id) && $id != 0) {
            $this->data['data'] = $this->Work_Speciality_Model->get(['id' => $id], true);
        ...
        $this->data['pagename'] = 'Set Work Speciality'; 
        $this->template->content->view('admin/workSpeciality/form', $this->data);
        $this->template->publish();
    }
    ...
    public function delete($id = 0){
        $data['status'] = "2";
        if (!empty($id) && $id != 0) {
            $response = $this->Work_Speciality_Model->setData($data, $id);
        } 
        if ($response != "") {
    ...
    public function checkName($id = 0){
        if ($this->input->server('REQUEST_METHOD') == 'GET') {
            ...
            $exist = $this->Work_Speciality_Model->get($request);
            if(!empty($exist)){
