...
class Insurance extends MY_Controller {
    ...
    public function __construct() {
        ...
        $this->data['page'] = ['name'=>"Insurance Type",'url'=>base_url('admin/Insurance'),'menu'=>'Insurance','submenu'=>'Insurance'];
        $this->data['pageTitle'] = ['title' => 'Insurance Type', 'icon' => ''];
    }
    ...
    public function index(){
        ...
        $this->data['pagename'] = 'Insurance Type List';
        $this->data['data'] = [];
    	$this->template->content->view('admin/insurance/manage', $this->data);
    ...
    public function getList() {
        ...
        $query['status'] = [0,1];
        $resultData =  $this->Insurance_Type_Model->get(array_merge($paggination,$query));
        $totalData =  $this->Insurance_Type_Model->get($query, false, true);
        $result['data'] = [];
    ...
    public function set($id = null) {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if (isset($data['id']) && !empty($data['id'])) {
                $this->Insurance_Type_Model->setData($data, $data['id']);
            } else {
                $this->Insurance_Type_Model->setData($data);
            }
        ...
        if (!empty($id) && $id != 0) {
            $this->data['data'] = $this->Insurance_Type_Model->get(['id' => $id], true);
        ...
        $this->data['pagename'] = 'Set Insurance Type'; 
        $this->template->content->view('admin/insurance/form', $this->data);
        $this->template->publish();
    }
    ...
    public function delete($id = 0){
        $data['status'] = "2";
        if (!empty($id) && $id != 0) {
            $response = $this->Insurance_Type_Model->setData($data, $id);
        } 
        if ($response != "") {
    ...
    public function checkName($id = 0){
        if ($this->input->server('REQUEST_METHOD') == 'GET') {
            ...
            $exist = $this->Insurance_Type_Model->get($request);
            if(!empty($exist)){
