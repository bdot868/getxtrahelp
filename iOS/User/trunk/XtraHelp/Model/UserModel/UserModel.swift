...
final class UserModel: NSObject,NSCoding {
    ...
    class func saveAboutLoved(with param: [String:Any]?, success withResponse: @escaping (_ user: UserModel, _ strMessage : String) -> Void, failure: @escaping FailureBlock) {
        
        
        SVProgressHUD.show()
        APIManager.makeRequest(with: AppConstant.API.ksaveAboutLoved, method: .post, parameter: param, success: {(response) in
            SVProgressHUD.dismiss()
            let dict = response as? [String:Any] ?? [:]
