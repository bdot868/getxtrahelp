...
class FAQModel: NSObject {
    ...
    class func getFAQList(with param: [String:Any]?,isShowLoader : Bool, success withResponse: @escaping (_ arr: [FAQModel],_ totalpage : Int,_ msg : String,_ ticketCount : Int) -> Void, failure: @escaping FailureBlock) {
        if isShowLoader {
        SVProgressHUD.show()
        }
        APIManager.makeRequest(with: AppConstant.API.kfaq, method: .post, parameter: param, success: {(response) in
            ...
            if  isSuccess,let dataDict = dict[kData] as? [[String:Any]] {
                let ticketCount : Int = Int(dict[kactiveSupportTicketCount] as? String ?? "0") ?? 0
                
                let arr = dataDict.compactMap(FAQModel.init)
                
                withResponse(arr,totalPages,message,ticketCount)
            }
            else {
