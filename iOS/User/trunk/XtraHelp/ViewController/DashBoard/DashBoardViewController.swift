...
extension DashBoardViewController {
    private func InitConfig(){
        ...
        self.vwTabCaregiver.lblTabName.text = TabbarItemType.Caregiver.name
        self.vwTabCaregiver.imgTab.image = TabbarItemType.Caregiver.img
        self.vwTabCaregiver.btnSelect.tag = TabbarItemType.Caregiver.rawValue
    ...
    private func setHomeTab() {
        ...
        if let vc = st.getViewController(with: VCIdentifier.HomeViewController) as? HomeViewController {
            self.addChild(vc)
            vc.view.frame = CGRect(x: 0, y: 0, width: self.vwContainer.frame.width, height: self.vwContainer.frame.height)//self.vwContainer.frame
    ...
    private func setMyjobTab() {
        ...
        if let vc = st.getViewController(with: VCIdentifier.MyJobParentViewController) as? MyJobParentViewController {
            self.addChild(vc)
            vc.view.frame = CGRect(x: 0, y: 0, width: self.vwContainer.frame.width, height: self.vwContainer.frame.height)//self.vwContainer.frame
            vc.view.backgroundColor = UIColor.clear
            self.vwContainer.addSubview(vc.view)
    ...
    private func setCaregiverTab(){
        ...
        let st = UIStoryboard.getStoryboard(for: UIStoryboard.Name.carGiver.rawValue)

        if let vc = st.getViewController(with: VCIdentifier.CarGiverViewController) as? CarGiverViewController {
            self.addChild(vc)
            vc.view.frame = CGRect(x: 0, y: 0, width: self.vwContainer.frame.width, height: self.vwContainer.frame.height)//self.vwContainer.frame
            vc.view.backgroundColor = UIColor.clear
            self.vwContainer.addSubview(vc.view)
    ...
    private func setFeedTab() {
        ...
        if let vc = st.getViewController(with: VCIdentifier.FeedViewController) as? FeedTabListViewController {
            self.addChild(vc)
            vc.view.frame = CGRect(x: 0, y: 0, width: self.vwContainer.frame.width, height: self.vwContainer.frame.height)//self.vwContainer.frame
            vc.view.backgroundColor = UIColor.clear
            self.vwContainer.addSubview(vc.view)
