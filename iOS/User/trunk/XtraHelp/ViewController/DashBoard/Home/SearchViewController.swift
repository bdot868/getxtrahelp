...
class SearchViewController: UIViewController {
    ...
    var arrNearestXaregiver : [CaregiverListModel] = []
    private var arrResult : [CaregiverListModel] = []
    ...
    var selecetdCategory : JobCategoryModel?
...
fileprivate extension SearchViewController{
    private func InitConfig(){
        ...
        self.tblResults?.register(MyJobListCell.self)
        ...
        if !self.isFromSearchCaregiver {
            self.getSearchList()
        }
        self.vwSuggetionMain?.isHidden = self.arrNearestXaregiver.isEmpty || self.isFromSearchCaregiver
        self.vwSearchMain?.isHidden = self.isFromSearchCaregiver
        self.vwReceentSearchMain?.isHidden = self.isFromSearchCaregiver
    ...
    private func configureNavigationBar() {
        ...
        appNavigationController?.appNavigationControllersetTitleWithBackWithColor(title: "", TitleColor: UIColor.CustomColor.textHedareLogin, navigationItem: self.navigationItem, isshowBackButton: true, isShowDirectHome: self.isFromSearchCaregiver)
        
        self.title = "Find CareGivers"
...
extension SearchViewController : UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.vwCaregiverRateMain?.isHidden = false
        cell.vwApplicationMain?.isHidden = true
        cell.vwSeprator?.backgroundColor = UIColor.clear
        if self.arrResult.count > 0 {
            cell.setCateGiverSearchData(obj: self.arrResult[indexPath.row])
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if self.arrResult.count > 0 {
            ...
            self.appNavigationController?.push(ProfileViewController.self,configuration: { vc in
                vc.selectedCaregiverID = self.arrResult[indexPath.row].id
                vc.isFromSearchCaregiver = self.isFromSearchCaregiver
                vc.selectedJobID = self.searchCaregiverJobID
            })
        }
    }
}
...
extension SearchViewController : UICollectionViewDelegate,UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if collectionView == self.cvReceentSearch {
            ...
            return CGSize(width: fontsize + 70.0, height: 50.0)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        print(indexPath.row)
        /*if collectionView == self.cvTopMentor {
            self.appNavigationController?.push(MentorProfileViewController.self)
        } else if collectionView == self.cvUpcomingSession {
...
extension SearchViewController {
    ...
    private func getCaregiverList(isshowloader :Bool = true){
        if let user = UserModel.getCurrentUserFromDefault() {
            ...
            if let category = self.selecetdCategory {
            ...
            CaregiverListModel.getCaregiverList(with: param, isShowLoader: isshowloader,  success: { (arr,totalpage,msg)  in
                ...
                self.tblResults?.es.stopPullToRefresh(ignoreDate: false, ignoreFooter: false)
                self.totalPages = totalpage
                
                self.arrResult.append(contentsOf: arr)
                self.hideFooterLoading(success: self.pageNo <= self.totalPages ? true : false )
                self.pageNo = self.pageNo + 1
                self.vwNoDataMain?.isHidden = !self.arrResult.isEmpty
                self.tblResults?.reloadData()
                self.lblNoData?.text = "No CareGivers Available!"
            }, failure: {[unowned self] (statuscode,error, errorType) in
                print(error)
                self.tblResults?.es.stopPullToRefresh(ignoreDate: false, ignoreFooter: false)
