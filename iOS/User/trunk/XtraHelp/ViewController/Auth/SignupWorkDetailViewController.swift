...
////                    vc.selectedDisabilitiesData = data
////                }
////            })
//        case .Experience:
...
//        self.vwDisabilitiesType?.txtInput.text = obj.name
//    }
//
    ...
    func saveWorkDetail() {
        ...
        for i in stride(from: 0, to: self.arrWorkExperience.count, by: 1) {
            ...
            let dict : [String:Any] = [
                ...
                kendDate : obj.endDate.getDateFromString(format: AppConstant.DateFormat.k_MMMMyyyy).getTimeString(inFormate: AppConstant.DateFormat.k_MM_yyyy),
        ...
        //self.view.endEditing(true)
        if let user = UserModel.getCurrentUserFromDefault(),let speciality = self.selectedWorkSpecialityData, let methodTransportation = self.selectedWorkMethodTransportationData,let disability = self.selectedDisabilitiesData {
            let dict : [String:Any] = [
                ...
                kworkDisabilitiesWillingTypeId : disability.workDisabilitiesWillingTypeId,
                kexperienceOfYear : self.vwExperience?.txtInput.text ?? "",
                kinspiredYouBecome : self.vwExperience?.txtInput?.text ?? "",
