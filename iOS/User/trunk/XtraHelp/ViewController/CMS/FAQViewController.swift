...
class FAQViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if XtraHelp.sharedInstance.isReloadFAQData {
            XtraHelp.sharedInstance.isReloadFAQData = false
            self.reloadFAQData()
        }
    }
...
extension FAQViewController {
    private func getFaqList(isshowloader :Bool = true){
        if let user = UserModel.getCurrentUserFromDefault() {
            ...
            FAQModel.getFAQList(with: param,isShowLoader : isshowloader, success: { (arr,totalpage,msg,ticketCount) in
                ...
                self.vwActiveDot?.isHidden = ticketCount == 0
                self.lblActiveCount?.text = ticketCount > 10 ? "9+" : "\(ticketCount)"
