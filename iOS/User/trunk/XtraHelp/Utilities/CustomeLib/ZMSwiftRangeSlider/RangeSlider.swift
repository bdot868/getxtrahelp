...
open class RangeSlider: UIControl {
    ...
    @IBInspectable open var displayTextFontSize: CGFloat = 14.0 {
        didSet {
            ...
            displayTextFont = UIFont.RubikBold(ofSize: displayTextFontSize)
            updateLayerFrames()
        }
    }
    ...
    func updateLayerFrames() {
        ...
        if labelsAreBelow {
            minValueDisplayLayer.frame = CGRect(x: minValuePosition,
                                                y: 30.0,//offsetY + max(thumbSize, minValueSize.height),
                                                width: max(thumbSize, minValueSize.width),
                                                height: displayTextFontSize)
        } else {
            minValueDisplayLayer.frame = CGRect(x: minValuePosition,
                                                y: (bounds.height/2.0) - 6.0,//offsetY - minValueSize.height,
                                                width: max(thumbSize, minValueSize.width),
                                                height: displayTextFontSize)
        }
        ...
        if labelsAreBelow {
            maxValueDisplayLayer.frame = CGRect(x: maxValuePosition,
                                                y: 30.0,//offsetY + max(thumbSize, maxValueSize.height),
                                                width: max(thumbSize, maxValueSize.width),
                                                height: displayTextFontSize)
        } else {
            maxValueDisplayLayer.frame = CGRect(x: maxValuePosition,
                                                y: (bounds.height/2.0) - 6.0,//offsetY - maxValueSize.height,
                                                width: max(thumbSize, maxValueSize.width),
                                                height: displayTextFontSize)
        }
