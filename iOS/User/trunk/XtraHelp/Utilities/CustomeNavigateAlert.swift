...
class CustomeNavigateAlert: NSObject {
    ...
    func openMapNavigation(_ sender:Any,_ pickertitle : String,AppoitnmentLatitude : Double,AppoitnmentLongitude : Double) {
        ...
        {
            (alert: UIAlertAction!) -> Void in
            let openurl : String = "comgooglemaps://?saddr=\(currentlat),\(currentlong)&daddr=\(AppoitnmentLatitude),\(AppoitnmentLongitude)"
            //"comgooglemaps://?saddr=\(currentlat),\(currentlong)&daddr=\(AppoitnmentLatitude),\(AppoitnmentLongitude)&dirflg=d"
