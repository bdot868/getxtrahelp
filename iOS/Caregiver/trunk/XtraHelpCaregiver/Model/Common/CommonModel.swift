...
class CommonModel: NSObject {
    ...
    class func getCommonData(with param: [String:Any]?,isShowLoader : Bool = true, success withResponse: @escaping (_ arrHearAbout: [HearAboutUsModel],_ arrlicenceType: [licenceTypeModel],_ arrInsuranceType: [InsuranceTypeModel],_ arrWorkSpeciality: [WorkSpecialityModel],_ arrWorkMethodOfTransportation: [WorkMethodOfTransportationModel],_ arrWorkDisabilitiesWillingType: [WorkDisabilitiesWillingTypeModel],_ msg : String) -> Void, failure: @escaping FailureBlock) {
        if isShowLoader {
            SVProgressHUD.show()
        }
        APIManager.makeRequest(with: AppConstant.API.kgetCommonData, method: .post, parameter: param, success: {(response) in
            ...
            if  isSuccess,let dataDict = dict[kData] as? [String:Any] {
                ...
                withResponse(arrHearAbout,arrlicenceType,arrInsuranceType,arrWorkSpeciality,arrWorkMethodOfTransportation,arrWorkDisabilitiesWillingType,message)
            }
            else {
