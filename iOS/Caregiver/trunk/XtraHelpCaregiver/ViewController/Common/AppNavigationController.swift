            ...
            if let notificationdata : [String : Any] = notification.object as? [String : Any] {
                ...
                if let msgdic = notificationdata["messages"] as? [String:Any], let msgdata = msgdic["messageData"] as? [String:Any] {
                    if let modeldtata = msgdata["model"] as? String {
                        if modeldtata == "acceptJobRequestByUser" {
                        ...
                        } else if modeldtata == "rejectJobRequestByUser" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                                self.push(JobDetailViewController.self, configuration: { (vc) in
                                    vc.selectedJobID = "\(modelid)"
                        ...
                        } else if modeldtata == "caregiverSubstituteJobByCaregiver" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                                self.push(JobDetailViewController.self, configuration: { (vc) in
                                    vc.selectedJobID = "\(modelid)"
                        ...
                        } else if modeldtata == "addMoneyInYourWalletForUserJobPayment" || modeldtata == "transactionSuccessForJobPayment" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                                self.push(JobDetailViewController.self,configuration: { vc in
                        ...
                        } else if modeldtata == "alertCaregiverMessageBeforeStartjobOf30Mint" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                                self.push(JobDetailViewController.self, configuration: { (vc) in
                                    vc.selectedJobID = "\(modelid)"
                        ...
                        } else if modeldtata == "sendJobImageRequest" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                                self.push(JobDetailViewController.self, configuration: { (vc) in
                                    vc.selectedJobID = "\(modelid)"
                        ...
                        } else if modeldtata == "sendJobVideoRequest" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                                self.push(JobDetailViewController.self, configuration: { (vc) in
                                    vc.selectedJobID = "\(modelid)"
                        ...
                        } else if modeldtata == "ongoingJobMediaRequestOfCaregiver" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                                self.push(JobDetailViewController.self, configuration: { (vc) in
                                    vc.selectedJobID = "\(modelid)"
                        ...
                        } else if modeldtata == "caregiverSubstituteJobRequestByCaregiver" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                                ...
                                /*self.push(JobDetailViewController.self, configuration: { (vc) in
                                    vc.selectedJobID = "\(modelid)"
                                    vc.selecetdTab = .Substitute
                                    vc.isFromHomeScreen = true
                                    vc.isFromNearestJob = true
                                    vc.isFromTabbar = false
                                })*/
                            }
                        } else if modeldtata == "caregiverSubstituteJobRequestAcceptByUser" || modeldtata == "caregiverSubstituteJobRequestAcceptByCaregiver" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                        ...
                        } else if modeldtata == "caregiverSubstituteJobRequestRejectByUser" || modeldtata == "caregiverSubstituteJobRequestRejectByCaregiver"{
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                        ...
                        }  else if modeldtata == "sendExtraTimeRequestOfCurrentJobByUser" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                                self.push(JobDetailViewController.self,configuration: { vc in
                                    vc.selecetdTab = .Upcoming
                                    vc.selectedJobID = "\(modelid)"
                                    vc.isFromHomeUpcomingScreen = true
                                    vc.isFromNearestJob = false
                                    vc.isFromTabbar = false
                                    vc.isFromJobAddHoursRequest = true
                                })
                            }
                        } else if modeldtata == "cancelUpcomingJobByAutoSystemForCaregiver" {
                            if let modelid = msgdata[kmodel_id] as? Int {
                                self.detachLeftSideMenu()
                                XtraHelp.sharedInstance.isMoveToTabbarScreen = .MyJobs
                        ...
                        } else if modeldtata == "sendAwardJobRequestByUser" {
                            self.detachLeftSideMenu()
                            XtraHelp.sharedInstance.isMoveToTabbarScreen = .MyJobs
...
//        .configure(barTintColor: .clear, tintColor: #colorLiteral(red: 1, green: 0.7725490196, blue: 0.3254901961, alpha: 1))
            ...
            if let user = UserModel.getCurrentUserFromDefault() {
                if !user.token.isEmpty {
                    ...
                    if user.profileStatus == .Default || user.profileStatus == .Subscription{
                        ...
                        self.push(SignUpLicensesViewController.self,animated: true,configuration: { (vc) in
                            vc.isFromLogin = true
                        })
                        ...
                        self.push(MyCalenderViewController.self,animated: true,configuration: { (vc) in
                            vc.isFromLogin = true
                        })
    ...
    func setUpNavigationBackWithTitleButton(title : String,TitleColor : UIColor,navigationItem : UINavigationItem) {
     ...
     func setUpNavigationJobStart(navigationItem : UINavigationItem) {
         
         let backBarButtonItem = UIBarButtonItem.itemWithBackPic(colorfulImage: #imageLiteral(resourceName: "ic_NavBackTabColor"), target: self, action: #selector(self.backbuttonClicked(sender:)))
         
         navigationItem.leftBarButtonItems = [backBarButtonItem]
         
         let chatBarButtonItem = UIBarButtonItem.itemWithBackPic(colorfulImage: #imageLiteral(resourceName: "Chat_Icon"), target: self, action: #selector(self.rightNextbuttonClicked(sender:)))
         navigationItem.rightBarButtonItems = [chatBarButtonItem]
     }
    ...
    func setUpBackTitleRightBackBtnNavigationBar(title : String,TitleColor : UIColor,rightBtntitle : String,rightBtnColor : UIColor,navigationItem : UINavigationItem) {
        ...
        let backBarButtonItem = UIBarButtonItem.itemWithBackPic(colorfulImage: #imageLiteral(resourceName: "Icon_Back_"), target: self, action: #selector(self.backbuttonClicked(sender:)))
        navigationItem.leftBarButtonItems = [backBarButtonItem,titleBarButtonItem]
...
extension AppNavigationController: AppSideMenuViewControllerDelegate {
    ...
    func setNavigationBackTitleJobStartNavigationBar(navigationItem : UINavigationItem) {
        
        self.setUpNavigationJobStart(navigationItem: navigationItem)
    }
