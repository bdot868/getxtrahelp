    ...
    private func InitConfig(){
        ...
        if self.isFromEditProfile {
            if let obj = self.selectedUserProfileData {
                self.setMapMarker(selLat: Double(obj.latitude) ?? 0.0, selLong: Double(obj.longitude) ?? 0.0, isUpdateValue: true)
            } else {
...
extension SignupLocationViewController {
    ...
    @IBAction func btnContinueClicked(_ sender: UIButton) {
        ...
        if (self.lblLocation?.text?.isEmpty ?? true){
            ...
            if self.isFromEditProfile {
                self.saveAddressOrLocation(isSave: true)
            } else {
                self.saveAddressOrLocation(isSave: false)
            }
        }
    }
...
extension SignupLocationViewController{
    ...
    func saveAddressOrLocation(isSave : Bool = true) {
        self.view.endEditing(true)
        if let user = UserModel.getCurrentUserFromDefault() {
            var dict : [String:Any] = [
            ...
            if !self.isFromEditProfile {
                dict[kprofileStatus] = "4"
            }
            
            let param : [String:Any] = [
                kData : dict
            ]
            UserModel.saveSignupProfileDetails(with: param,type: .YourAddress, success: { (model, msg) in
                if isSave {
                    if self.isFromEditProfile {
                    ...
                    self.appNavigationController?.push(SignupWorkDetailViewController.self,configuration: { (vc) in
