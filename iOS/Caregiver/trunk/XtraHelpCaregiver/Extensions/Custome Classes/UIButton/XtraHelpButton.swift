...
class XtraHelpButton : UIButton
{
    ...
    func setupView()
    {
        ...
        delay(seconds: 0.1, execute: {
            self.backgroundColor = GradientColor(gradientStyle: .topToBottom, frame: self.frame, colors: [UIColor.CustomColor.buttonBGGCOne,UIColor.CustomColor.gradiantColorBottom])
        })
        delay(seconds: 0.1, execute: {
