...
class CalenderView: UIView {
    ...
    var arrPrevCaregiverAvailibilityDates : [Date] = [] {
        didSet{
            self.calendar?.reloadData()
        }
    }
    
    var isReloadCalender : Bool = false {
        didSet {
            //self.selectedDates = []
            if self.selectedDates.count > 0 {
                for i in stride(from: 0, to: self.selectedDates.count, by: 1) {
                    self.calendar?.deselect(self.selectedDates[i])
    ...
    var isFromAvailibilityRecurring : Bool = false {
        didSet{
            self.calendar?.allowsMultipleSelection = !isFromAvailibilityRecurring
        }
    }
    ...
    private func ConfigureCalendarView() {
        if let calendar = self.calendar {
            ...
            calendar.appearance.titleTodayColor = UIColor.CustomColor.AvailableGroupSessionStatusColor
            ...
            calendar.placeholderType = .none
            calendar.appearance.borderRadius = 1.0
            calendar.appearance.eventSelectionColor = UIColor.CustomColor.appColor
...
extension CalenderView : FSCalendarDelegate, FSCalendarDataSource, FSCalendarDelegateAppearance{
    ...
    func calendar(_ calendar: FSCalendar, appearance: FSCalendarAppearance, fillDefaultColorFor date: Date) -> UIColor? {
        ...
        let dateString = date.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)
        let filter = self.selectedDates.map({$0.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)})
        let filterAvailibility = self.arrAvailibilitySelectedDates.map({$0.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)})
        
        if filter.contains(dateString) {
        ...
        } else if filterAvailibility.contains(dateString) {
    ...
    func calendar(_ calendar: FSCalendar, didSelect date: Date, at monthPosition: FSCalendarMonthPosition) {
        print("Select Date : \(date.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)))")
        //let selctdate = self.dateFormatter1.string(from: date)
        //calendar.appearance.titleSelectionColor = #colorLiteral(red: 0.8823529412, green: 0.6784313725, blue: 0.003921568627, alpha: 1)
        /*if isFromCreateJobScreen {
            self.selectAvailibilityDate = ""
            self.selectedDates.append(selctdate)
            self.delegate?.selectMultipleCalenderDate(arr: self.selectedDates)
        } else {

            self.calenderView.deselect(self.selectAvailibilityDate.getDateFromString(format: AppConstant.DateFormat.k_yyyy_MM_dd))
            if !self.isFromTempSelectSignleDate {
                self.selectAvailibilityDate = self.dateFormatter1.string(from: date)
                self.selectedDates.removeAll()
            }
            
        }*/
        //self.selectedDates.append(self.selectAvailibilityDate)
        //if self.selectFromHomeScreenDate != "" {
            //self.calenderView.deselect(date)
            //self.selectFromHomeScreenDate = ""
       ...
       // }
        ...
        if self.isFromCaregiverAvailibility {
            ...
            self.calendar?.reloadData()
            self.delegate?.didSelectCalenderDate(date: date)
        } else {
            self.selectedDates.append(date)
            self.calendar?.reloadData()
            self.delegate?.selectMultipleCalenderDate(arr: self.selectedDates)
    ...
    func calendar(_ calendar: FSCalendar, didDeselect date: Date, at monthPosition: FSCalendarMonthPosition) {
        print("DeSelect Date : \(date.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)))")
        //let deselctdate = self.dateFormatter1.string(from: date)
        //if isFromCreateJobScreen {
           // self.selectAvailibilityDate = ""
        let filter = self.selectedDates.filter({($0.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)) == (date.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd))})
            if filter.count > 0 {
                self.selectedDates.removeAll(where: {$0.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd) == date.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)})
                print(self.selectedDates)
                self.delegate?.selectMultipleCalenderDate(arr: self.selectedDates)
                self.calendar?.reloadData()
            }
        self.delegate?.didDeSelectCalenderDate(date: date)
        //}
    }
    ...
    func calendar(_ calendar: FSCalendar, appearance: FSCalendarAppearance, eventDefaultColorsFor date: Date) -> [UIColor]? {
        ...
        print(tomorrow.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd))*/
        //let prevdate : [String] = self.arrPrevCaregiverAvailibilityDates.map({$0.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)})
        ...
        //if self.selectedDates.contains(date) {
            ...
            let filterprev = self.arrPrevCaregiverAvailibilityDates.map({$0.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)})
            //let filter = self.arrPrevCaregiverAvailibilityDates.filter({$0 == date})
            let filter = filterprev.filter({$0 == dateString})
            color =  (filter.count > 0) ? [UIColor.CustomColor.appColor] : [UIColor.clear]
        } else if prevdate.contains(dateString) {//self.arrPrevCaregiverAvailibilityDates.contains(date){//prevdate.contains(date.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)) {
            //date.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd) == tomorrow.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd) {
            //return [UIColor.CustomColor.tabBarColor]
            //let filter = self.arrPrevCaregiverAvailibilityDates.filter({$0 == date})
            let filterprev = self.arrPrevCaregiverAvailibilityDates.map({$0.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)})
            let filter = filterprev.filter({$0 == dateString})
    ...
    func calendar(_ calendar: FSCalendar, numberOfEventsFor date: Date) -> Int {
        ...
        if filterprev.contains(dateString){
            //let filter = self.arrPrevCaregiverAvailibilityDates.filter({$0 == date})
            let filterprev = self.arrPrevCaregiverAvailibilityDates.map({$0.getFormattedString(format: AppConstant.DateFormat.k_yyyy_MM_dd)})
            let filter = filterprev.filter({$0 == dateString})
