...
class SubstituteAdapter(val context: Context) :
    RecyclerView.Adapter<SubstituteAdapter.ViewHolder>() {
    var jobList: ArrayList<SubstituteListResponse.Data> = ArrayList()

    lateinit var acceptRejectSubJobClickListener: AcceptRejectSubJobClickListener

    @JvmName("setAdapterList")
    fun setAdapterList(list: ArrayList<SubstituteListResponse.Data>) {
    ...
    override fun onCreateViewHolder(
        ...
        val view =
            LayoutInflater.from(parent.context).inflate(R.layout.adapter_substitute, parent, false)
        ...
        holder.itemView.setOnClickListener {
            context.startActivity(
                Intent(context, CaregiverProfileActivity::class.java)
                    .putExtra(CaregiverProfileActivity.CAREGIVERID, data.newCaregiverId)
            )
        }
            ...
            .setPositiveButton(
                ...
                if (isAccept) {
                    acceptRejectSubJobClickListener.onSubClick(
                        jobList[position].userJobSubstituteRequestId,
                        true
                    )
                } else {
                    acceptRejectSubJobClickListener.onSubClick(
