...
import com.app.xtrahelpcaregiver.Utils.*
import com.app.xtrahelpcaregiver.apiCalling.RetrofitClient
import com.app.xtrahelpcaregiver.chatModule.MessageListener
...
open class BaseActivity : AppCompatActivity(), View.OnClickListener, SnackRetryInterface,
    PermissionUtils.PermissionSettingsListener, MessageListener {
    ...
    open var permissionUtils: PermissionUtils? = null
    val GALLERY = 1
    var CAMERA: Int = 2
    var path = ""
...
//                    finish()
//                }
                pref.getString(Const.profileStatus)
                    .toString() == "1" || pref.getString(Const.profileStatus).toString() == "0" -> {
                    startActivity(Intent(activity, PersonalDetailActivity::class.java))
                    finish()
                }
                pref.getString(Const.profileStatus).toString() == "2" -> {
                    startActivity(Intent(activity, CertificationsLicensesActivity::class.java))
                    finish()
                }
                pref.getString(Const.profileStatus).toString() == "3" -> {
                    startActivity(Intent(activity, YourAddressActivity::class.java))
                    finish()
                }
...
//                    val deviceToken: String = task.result.toString()
//                    deviceTokenReturn = deviceToken;
