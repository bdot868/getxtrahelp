...
import com.app.xtrahelpcaregiver.Adapter.ReviewAdapter
...
class CompleteProfileActivity : BaseActivity() {
    ...
    var workExperienceList: ArrayList<WorkExperienceData> =
        ArrayList()
    ...
    private fun init() {
        ...
        if (!TextUtils.isEmpty(caregiverData)) {
            ...
            if (data.workExperienceData.isEmpty()) {
                ...
                for (i in data.workExperienceData.indices){
                    val d = data.workExperienceData[i]
                    val WE = WorkExperienceData(d.userWorkExperienceId,d.userId,d.workPlace,d.startDate,d.endDate,d.leavingReason,d.description)
